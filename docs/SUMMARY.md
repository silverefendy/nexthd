...
| 3 | Cek nasib 2 tiket uji di server | `TKT-2608-0006` (gagal di percobaan pertama, tanpa worklog) dan `TKT-2608-0007` (berhasil, ada worklog) — hapus kalau tidak dibutuhkan lagi untuk referensi |
| 4 | Bersihkan file script sementara di server | Banyak file `.py` di `/home/it/` dari sesi debugging Round 4 & investigasi sidebar 2 September (`patch_workflow_guard*.py`, `verify_migrate_kedua.py`, `test_worklog_pr11*.py`, `isolate_*.py`, `diag_*.py`, dll) — opsional, tidak mendesak |
| 5 | Audit berkala folder `nexthd/fixtures/` vs `hooks.py` | Menyusul temuan item MM (2 September) — cek apakah ada file fixture fisik usang lain yang masih tersisa untuk DocType yang pernah "dinonaktifkan dari fixtures" (mis. cross-check dengan riwayat `Workflow Transition` di item LL) |
...
*Dokumen ini dikelola oleh Claude. Update terakhir: 2026-09-06.*
