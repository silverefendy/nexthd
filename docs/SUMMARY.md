...
>
> **Last updated:** 2026-09-02 (Item MM — root cause final regresi sidebar "NextHD" 17→15 setiap `bench migrate` DITEMUKAN & DIPERBAIKI PERMANEN: file fixture usang `nexthd/fixtures/workspace_sidebar.json` masih ter-scan `sync_fixtures()` meski sudah dihapus dari daftar array `hooks.py`. Item KK/AA/LL "bench migrate uji tahan" yang sebelumnya tercatat pending kini resmi TUNTAS. Detail investigasi lengkap di `docs/BUG_WORKSPACE_SIDEBAR.md`)
