...
**Root cause regresi:** `Workspace Sidebar.standard` untuk record "NextHD" ternyata `0` (bukan
`1`). Sesuai `POLA_KERJA.md §1.C`, `standard` harus `1` agar perubahan sidebar permanen —
kondisi `standard=0` membuat sidebar rawan diregenerasi ulang murni dari `Workspace.links`
(yang tidak memuat "NextHD Reporting", karena item itu ditambahkan lewat UI langsung ke
`Workspace Sidebar Item`), kemungkinan dipicu proses `doc.save()` pada Workspace itu sendiri.
...
2. "NextHD Reporting" (`link_type: Workspace`, `link_to: NextHD Report`) ditambahkan kembali
   lewat UI — **catatan navigasi penting:** menu yang benar adalah ikon **panah ke bawah di
   kiri atas** halaman Workspace, BUKAN titik tiga "⋯" kanan atas seperti tercatat sebelumnya.
...
**Status:** Item DD ditutup tuntas. Commit: `933ec10` (dokumentasi), kode oleh Efendy.
...
**⚠️ PENDING:** `bench migrate` uji tahan belum pernah dijalankan sejak fix item DD & KK. Kedua
faktor risiko yang diketahui (`standard`, `app`) sudah diperbaiki untuk kedua record, tapi belum
dibuktikan empiris lewat migrate sungguhan. Bisa digabung dengan uji migrate final item AA.
